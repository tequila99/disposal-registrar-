...
class CardReaderNFC {
  ...
  async connect () {
    this.nfcLib.on('reader', reader => {
      ...
      this.cardReader.on('card', async card => {
        ...
        try {
          ...
          console.log(`Received response '${getStatusCode(r1)}' from card '${card.atr}' to request '${command1.toString('hex')}'`)

          if (isOkResponse(r1) === true) { // utf8 is default encoding
            const r2 = await this.cardReader.transmit(command2, 40)
            console.log(`Received response '${getStatusCode(r2)}' from card '${card.atr}' to request '${command2.toString('hex')}'`)

            if (isOkResponse(r2) === true) {
              const r3 = await this.cardReader.transmit(command3, 40)
              console.log(`Received response '${getStatusCode(r3)}' from card '${card.atr}' to request '${command3.toString('hex')}'`)
    ...
    this.nfcLib.on('error', err => {
      console.log('Произошла ошибка в процессе работы с картридером ', err.message)
    })
  }
}
